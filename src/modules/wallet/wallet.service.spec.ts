...
describe('WalletService', () => {
  ...
  describe('debit', () => {
    it('reduces available balance and writes debit ledger entry', async () => {
      ...
      expect(walletRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ availableBalance: 30000 }),
      );
    ...
    it('throws BadRequestException when balance is insufficient', async () => {
      ...
      await expect(
        service.debit('wallet-1', 200, LedgerAccount.ESCROW, 'txn-2'),
    ...
    it('throws NotFoundException when wallet does not exist', async () => {
      walletRepo.findOne.mockResolvedValue(null);
      await expect(
        service.debit('wallet-x', 100, LedgerAccount.ESCROW, 'txn-3'),
  ...
  describe('lock', () => {
    ...
    it('throws when available balance is insufficient', async () => {
      ...
      await expect(service.lock('wallet-1', 10000)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('handleDepositWebhook', () => {
    ...
    it('credits wallet and marks intent COMPLETED for pending deposit', async () => {
      ...
      dataSource.transaction.mockImplementation(
        async (fn: (em: any) => Promise<void>) => {
          await fn({
            getRepository: () => ({
              update: jest.fn(),
              findOne: jest.fn().mockResolvedValue(makeWallet(0)),
              save: jest.fn(),
              create: jest.fn((d) => d),
            }),
          });
